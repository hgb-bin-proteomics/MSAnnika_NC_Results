...
df <- df %>% 
  ...
  mutate(txt_position = ifelse(Type == "True XL", round(meanTrue / 2), round(meanTrue + meanFalse) + 15)) %>% 
...
ggplot(df, aes(x=Tool, y=Mean, fill=Type)) +
  ...
  geom_errorbar(aes(x=Tool, ymin=errorbar_position-SD, ymax=errorbar_position+SD), 
                colour = rep(c("#40916c", "#ef233c"), 2), width = 0.3, stat = "identity", linewidth = 1.0,
  ...
  xlab("MS Annika search approach") +
  ylab("Number of identified crosslinks (mean) at 1% estimated FDR") +
  ylim(c(0, 260)) +
  labs(fill="Crosslinks") +
  geom_text(aes(y=txt_position, label=round(Mean, 0)), color = rep(c("white", "black"), 2), size=5.0) +
  geom_text(aes(y = 255, label = paste0(round(FDR, 2), "%")), size=5.0, angle = 0, hjust = 0.5) +
