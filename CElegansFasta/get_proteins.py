...
def get_proteins(df: pd.DataFrame) -> Set[str]:
...
def main(argv = None) -> None:
    s1 = pd.read_excel("S1_all_ng_linear_Amanda_deisotope_decharge.xlsx")
    s2 = pd.read_excel("S2_all_ng_linear_Amanda_deisotope_decharge.xlsx")
    s3 = pd.read_excel("S3_all_ng_linear_Amanda_deisotope_decharge.xlsx")
    s1_proteins = get_proteins(s1)
    print(f"Found {len(s1_proteins)} in S1.")
    s2_proteins = get_proteins(s2)
    print(f"Found {len(s2_proteins)} in S2.")
    s3_proteins = get_proteins(s3)
