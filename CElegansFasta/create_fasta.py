...
def main(argv = None) -> None:
    ...
    with open("proteins.json", "r", encoding = "utf-8") as f:
        ...
        f.close()
    print(f"Loaded {len(proteins)} proteins from json (should 3069).")
    print(len(proteins) == 3069)
    ...
    for protein in fasta_proteins:
        for keep in proteins:
            if keep in protein.id:
                ...
                break
    print(f"Found {len(proteins_to_keep)} proteins to keep (should be 3069).")
    print(len(proteins_to_keep) == 3069)
    SeqIO.write(proteins_to_keep, "filtered_uniprotkb_proteome_UP000001940_2024_03_15.fasta", "fasta")
    print("Wrote fasta file: filtered_uniprotkb_proteome_UP000001940_2024_03_15.fasta")
