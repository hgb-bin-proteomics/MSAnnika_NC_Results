...
def clean_sequence(merox_peptide):
    merox_peptide = str(merox_peptide).replace("[", "").replace("]", "").replace("{", "").replace("}", "")
    merox_peptide = merox_peptide.replace("B", "C")
    merox_peptide = merox_peptide.upper().strip()
    for i in range(10):
        if str(i) in merox_peptide:
            raise RuntimeError(f"Found invalid symbol in peptide sequence: {merox_peptide}")
    if merox_peptide == "":
        raise RuntimeError("Found empty peptide sequence.")
